...
def set_name(player: Player):
    # 1回だけparticipantの名前を記録。すると、参加者の名前を一括で記録できる
...
class Quiz(Page):
    ...
    def vars_for_template(player):
        return dict(
            ...
            question = Constants.trial[Subsession.order[player.round_number - 1]][0]
        )
    # データ送るための関数てきなやつ
    @staticmethod
...
def correct_or_not(group: Group):
    players = group.get_players()
    for player in players:
        if player.answer == Constants.trial[Subsession.order[player.round_number - 1]][1]:
